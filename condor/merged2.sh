...
# --- 1. Rutas ---
EOS_BASE="/eos/cms/store/group/phys_heavyions/jvelazqu/energycorr/event_sel_studies/pileupfilter"
OUTPUT_DIR="/eos/cms/store/group/phys_heavyions/jvelazqu/energycorr/merged/"
OUTPUT_FILE="${OUTPUT_DIR}/merged_R4_axiWTA_cen_OO_minimunbias.root"
mkdir -p "${OUTPUT_DIR}"
...
# --- 2. Encontrar TODOS los _processed.root (find, no glob de shell) ---
echo "==> Buscando archivos en ${EOS_BASE}/000*"
mapfile -t ALL_FILES < <(find "${EOS_BASE}"/000* -name 'L1_object_data_*_processed.root' 2>/dev/null | sort -V)
