...
# --- 1. Rutas ---
EOS_BASE="/eos/cms/store/group/phys_heavyions/jvelazqu/energycorr/eecAnalysis_ppref_2024-09-10/eecOOHistograms/crab_eecAnalysis_ppref_2024-09-10/260910_041527"
OUTPUT_DIR="/eos/cms/store/group/phys_heavyions/jvelazqu/energycorr/merged/"
OUTPUT_FILE="${OUTPUT_DIR}/merged_eecppRef_2024-09-10.root"
mkdir -p "${OUTPUT_DIR}"

GOOD_LIST=$(mktemp)
BAD_LIST="${OUTPUT_DIR}/bad_files_ppref.txt"
: > "${BAD_LIST}"

# --- 2. Encontrar TODOS los archivos de output (find, no glob de shell) ---
echo "==> Buscando archivos en ${EOS_BASE}/000*"
mapfile -t ALL_FILES < <(find "${EOS_BASE}"/000* -name 'eecAnalysis_ppref_2024-09-10_*.root' 2>/dev/null | sort -V)
